...
namespace cplex_tap {
    ...
    ILOMIPINFOCALLBACK1(VPLS_HAMMING_INFO_CBK, IloInt, num) {
        IloEnv env = getEnv();
    ...
    SolverVPLSHamming::solve_and_print(int dist_bound, int time_bound, bool progressive, bool debug, bool production,
        ...
        if (seed) {
            ...
            //vector<int> warm_sol = GreedyHeuristic::getBasicSolution(tap, time_bound, dist_bound);
            //cout << "computed start solution" << endl;
            //warm_start(warm_sol, env, n, x, s, cplex);
        }
