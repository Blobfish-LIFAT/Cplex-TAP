...
namespace cplex_tap {
    ...
    Solution pricingSolver::solve() const {
        ...
        rmpObj = rmpSol.z;
        std::cout << "[STEP][END] Building RMP model" << std::endl;
        ...
        for (int i = 0; i < pricingIST.getNbDims(); ++i) {
            ...
            for (int j = 0; j < pricingIST.getAdSize(i); ++j) {
                cpLeftSel[i][j] = IloNumVar(cplex, 0, 1, IloNumVar::Bool, ("l_" + std::to_string(i) + "," + std::to_string(j)).c_str());
                cpRightSel[i][j] = IloNumVar(cplex, 0, 1, IloNumVar::Bool, ("r_" + std::to_string(i) + "," + std::to_string(j)).c_str());
        ...
        for (int i = 0; i < pricingIST.getNbDims(); ++i) {
            pricing.add(IloRange(cplex, 0, cpSelection[i] + cpGroupBy[i], 1, ("sel_gk_" + std::to_string(i)).c_str()));
        }
        ...
        for (int i = 0; i < pricingIST.getNbDims(); ++i) {
            for (int j = 0; j < pricingIST.getAdSize(i); ++j) {
                ...
                // (13)
                pricing.add(IloRange(cplex,0,((cpLeftSel[i][j]+cpRightSel[i][j])/2.0) - cpSelExclusive[i][j], IloInfinity));
                expr += cpSelection[i];
                ...
                for (int k = 0; k < pricingIST.getNbMeasures(); ++k) {
                    pricing.add(IloRange(cplex,0,2 - cpSelExclusive[i][j] - cpLeftMeasure[k] - cpRightMeasure[k], IloInfinity));
                }
            }
        }
        /*
        ...
        bool isNewQuerySelected = cplex_solver.getValue(tap_s[rmpQSet.size()]);
